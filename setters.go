...
func (s bstore) Set(pKV KV) error {
	return s.Update(func(txn *badger.Txn) error {
		return txn.Set([]byte(pKV.prefix+pKV.key), []byte(pKV.value))
	})
}
...
func (s bstore) SetTTL(pKV KV, pTTLSecs int) error {
	return s.Update(func(txn *badger.Txn) error {
		entry := badger.NewEntry([]byte(pKV.prefix+pKV.key), []byte(pKV.value)).WithTTL(time.Second * time.Duration(pTTLSecs))
